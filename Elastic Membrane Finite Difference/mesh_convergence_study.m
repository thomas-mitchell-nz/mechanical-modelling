% ENME302
% Thomas Mitchell
% Convergence study of 1D FEA study of elastic membrane with linear and 
% quadratic element models
...
j=1; % iterator
uc = zeros(max_noe-1,1);
error = zeros(max_noe-1,1);
for noe=max_noe:-1:min_noe % number of elements 
    ...
    ubc = double(Kg\Fg);
    x = double(x);
    uc(j) = interp1(x,ubc',W/2); % interpolate central displacement

    % Displacement with finest mesh 
    if (noe == max_noe)
        uc_finest = uc(j); 
    end

    error(j) = abs(uc(j)-uc_finest)/uc_finest*100; % relative error
    j = j+1; % iterator
end

% Table
lin_conv = table;
lin_conv.Displacement_at_Centre = uc;
lin_conv.Number_of_Elements = [max_noe:-1:min_noe]';
lin_conv.Error = error;
lin_conv
...
% Table
quad_conv = table;
quad_conv.Displacement_at_Centre = uc;
quad_conv.Number_of_Elements = [max_noe:-1:min_noe]';
quad_conv.Error = error;
quad_conv
